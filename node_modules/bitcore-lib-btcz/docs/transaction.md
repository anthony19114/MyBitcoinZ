...
Note also that an output can be used only once. That's why there's a concept of "change address" in the bitcoin ecosystem: if an output of 10 BTCZ is available for me to spend, but I only need to transmit 1 BTCZ, I'll create a transaction with two outputs, one with 1 BTCZ that I want to spend, and the other with 9 BTCZ to a change address, so I can spend this 9 BTCZ with another private key that I own.
...
var transaction = new Transaction().fee(1e8);  // Generous fee of 1 BTCZ
```
